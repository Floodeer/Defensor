...
import com.floodeer.plugins.towerdefense.database.Callback;
import com.floodeer.plugins.towerdefense.database.Database;
import com.floodeer.plugins.towerdefense.database.SQLWriter;
...
public class DataManager {
    ...
    public DataManager() {
        database = Defensor.get().getDatabase();
    }
    ...
    public void loadPlayer(UUID player, GamePlayer gp) {
        ...
        new SQLWriter() {
            final Connection connection = database.getConnection().get();
        ...
        }.writeOperation(database.getExecutor(), Defensor.get().getLogger(), "Error while loading " + player.toString() + "'s data");
    }
    ...
    public void updatePlayerAsync(GamePlayer gp) {
        ...
        }.writeOperation(database.getExecutor(), Defensor.get().getLogger(), "Error while saving " + gp.getName() + "'s data");
    }
    ...
    private void insertPlayer(String uid) {
        ...
        try {
            ...
            String queryBuilder = "INSERT INTO `defensor_player` " +
                    ...
                    "VALUES " +
                    "(NULL, ?, ?);";
            preparedStatement = database.getConnection().get().prepareStatement(queryBuilder);
            preparedStatement.setString(1, uid);
            preparedStatement.setString(2, Defensor.get().getServer().getPlayer(uuid).getName());
            preparedStatement.executeUpdate();
